...
//            mModule = PyTorchAndroid.loadModuleFromAsset(getAssets(), "d2go.pt");
            //model load
            mModule = LiteModuleLoader.load(MainActivity.assetFilePath(getApplicationContext(), "traced_net2.ptl"));
//          traced_net : 941ms traced_net2: 875ms traced_optimized_net: ???
            //class load
...
//            Log.v("Yolact", "outputs len: " + outputs.size());
            ...
            ArrayList<Integer> closeidx = PrePostProcessor.closerange((int) nmsoutputs.size()/7, nmsoutputs, 0.7f);


            Log.v("Yolact", "closeidx: " + closeidx);
            final TextView closestuff1 = findViewById(R.id.textView);
            ArrayList<String> closelb = new ArrayList<String>();
            for (int i=0; i < closeidx.size(); i++) {
                closelb.add(PrePostProcessor.mClasses[closeidx.get(i)]);
                Log.v("Yolact", "closelb: " + PrePostProcessor.mClasses[closeidx.get(i)]);
            }
            closestuff1.setText(String.format("%s", closelb));
            ...
            for (int i = 0; i < count; i++) {
                for (int j = 0; j < 138; j++) {
                    for (int k = 0; k < 138; k++) {
                        ...
                        if (sum > 0.5) {
                            if ((j >= 138*nmsoutputs.get(i*7+1)) && (j <= 138*nmsoutputs.get(i*7+3))) {
                                if ((k >= 138*nmsoutputs.get(i*7+0)) && (k <= 138*nmsoutputs.get(i*7+2))) {
...
//            Log.v("Yolact", "resizedmask created");
            ...
            runOnUiThread(() -> {
                ...
                mResultView.setmBitmap(mBitmap);
        ...
        final long processTime = SystemClock.elapsedRealtime() - startTime2;
        Log.d("Yolact", "process time (ms): " + processTime);
//        Log.v("Yolact", "draw!");
    }
}
